...
public class Matching {
	...
	public static int[][] match(Dating d) {
		...
		HashMap<Integer, NetParticipant> Netmatchtable = d.getNetParticipants();
		boolean net = d.getNet();
		...
		if(net) {
			for(int i=1; i<=npart;i++){
				Participant p1 = Netmatchtable.get(i);
				amor[(p1.getID()-1)] = NetmatchCheck(p1, Netmatchtable, maxlike, gndarr);
			}
		}
		return amor;
	}
	...
	public static int[] NetmatchCheck(Participant p1, HashMap<Integer, NetParticipant> h, int maxlike, int[]gndarr) {
		int[] marr = p1.getMatches();
		int[] matches = new int[maxlike];
		int pid = p1.getID();
		int matchcount = 0;
	...
	public static String NetmatchToText(int[][] arr, HashMap<Integer, NetParticipant> h) {
		String nl = Processing.getLineSep();
		StringBuffer sb = new StringBuffer();
		sb.append("------------------------------------------------------------"+nl);
		for (int i=0; i<arr.length; i++) {
	...
	public static String NetprintDating(Dating sd){
		StringBuffer sb2 = new StringBuffer();
		int[][] ma = match(sd);
		sb2.append(sd.toString());
		HashMap<Integer, NetParticipant> hm = sd.getNetParticipants();
		sb2.append(NetmatchToText(ma, hm));
		return sb2.toString();
	}
